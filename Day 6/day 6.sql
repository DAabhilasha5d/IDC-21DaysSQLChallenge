...
SELECT service, COUNT(DISTINCT(patient_id)) AS No_of_patients  FROM patients
GROUP BY service;

-- 2. Calculate the average age of patients grouped by service.
SELECT service, ROUND(AVG(age), 0) AS Avg_age FROM patients
GROUP BY service;

-- 3. Find the total number of staff members per role.
SELECT role, COUNT(DISTINCT(staff_id)) AS Total_staff FROM staff
GROUP BY role;
